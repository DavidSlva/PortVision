...
const API_BASE_URL = `${BASE}/interpreter`; // Ajusta según tu configuración
...
>("volumenAnual/fetchVolumenTotalPorAnio", async (anio, thunkAPI) => {
  try {
    ...
    return thunkAPI.rejectWithValue(
      error.response?.data?.message ||
        "Error al cargar el volumen total por año."
    );
  }
});
...
>(
  ...
  async ({ codigo_puerto, anio }, thunkAPI) => {
    try {
      ...
      return thunkAPI.rejectWithValue(
        error.response?.data?.message ||
          "Error al cargar el volumen por puerto en el año."
      );
    }
  }
);
...
const volumenAnualSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder
      ...
      .addCase(
        fetchVolumenTotalPorAnio.fulfilled,
        (state, action: PayloadAction<VolumenTotalAnio[]>) => {
          state.loading = false;
    ...
    builder
      ...
      .addCase(
        fetchVolumenPorPuertoAnio.fulfilled,
        (state, action: PayloadAction<VolumenPorPuertoAnio[]>) => {
          state.loading = false;
